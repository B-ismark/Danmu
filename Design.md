...
deletability being what proves no sixth site was left behind. The `detect-pipeline` and
`off-square-cost` baselines are byte-identical across the change, which is the proof it
was a no-op on every centred room.

### …and the box around a room that is not one

§ 44 left `wallFrame` reading the polygon's **bounds**, which fixed the off-centre
rectangle — a wall drag plus Re-scan — and left an L, T or U measured to the box around
it. That is closed now, and it was bigger than the case before it.

`wallFrame` names a wall rather than a bound: of the walls whose INNER face is turned
toward this slot's camera and which reach the image's centre column, the nearest, provided
nothing stands between it and the lens. Measured at the dimensions the layout picker ships
— reachability is one press, since *"Photograph my real room first"* is offered for
whichever preset is selected — and printed on every green run:
...
standing in for a real ultrawide (`k` 0.49× too small) left a 700 mm print reading 790 mm
— +12.9%, indistinguishable from slop. Learn the lens, from EXIF or the vanishing-point
solve, and the same photograph reads +130.6%.

**The `u` is a mis-placed CAMERA, not a mis-measured wall**, and the sentence this section
used to carry about it was wrong: it said the notch puts the wall ahead of a north-facing
lens at `z = 0` against a bounds answer of `depth/2`. The notch's inner face is at
`−depth/2 + 0.5·depth`, exactly zero for **every** `u` room, so the rig stands the lens ON
that wall. There is no wall in front of it, the geometry answers null, and every site takes
its no-frame branch from the table above. Its other three views were never wrong and are
unchanged — a first probe of this claimed they were 130% out, having grazed the notch's
side walls, which the shipped selection rejects because a wall touching the centre column
blocks a point of the image and not the column. **What none of it fixes is the rig:**
standing the camera at the bounding box's centre is what puts it on a wall, and that is
filed as the larger item.

Two more things went with it. `wallSpan` is **deleted** — it outlived `wallDistance` by a
...
both questions with one predicate, and § 44's second filed residual closed with the first.
