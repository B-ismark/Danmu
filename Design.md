...
| `lib/sun-shadow.ts` | `castsSunShadow` — whether a piece may write into the sun's shadow map. **Walls only ever `receiveShadow`, never cast**, because the dollhouse view culls the near ones and a casting wall would black out the room the moment the camera came round. So the sun passes through the plaster, hits the back of a TV bolted to the far wall, and the TV — which does cast — drops a shadow on a floor the light never entered. The gate therefore belongs on the piece: a wall-rider casts only when the sun is on the same side of its wall as the room is, which is one dot product against `frontVector`. Gated on `ridesWall` and not `isWallMountedPart` — the latter is true for a ceiling fan (no wall to be behind) and for a door (whose leaf occluding the light is arguably right). A studio mood passes `null` and means “cast, as before”: its key light is a lighting rig, not something standing outside the building. The sign is the whole content, and a sign is invisible at 0°/180°, so `tests/sun-shadow.test.ts` asserts all four walls. **It reads `moodKeyDirection`, not `moodSunDirection`**, and that distinction was a bug for one commit: feeding it the sun function returned `null` for the two studio moods and exempted them, on the stated grounds that their key light is a lighting rig rather than something standing outside the building. The arithmetic refuses that — the rig is realised at `max(12, extent * 1.6)` metres, so `KEY_DIR · frontVector` is −0.390 on the south wall and −0.488 on the east, and the exemption left this exact bug standing on half the walls in `cool`, which carries the brightest ambient of the set. `null` now means only what it says: no key light at all, i.e. a sun below the horizon. The bearing still applies to the sun alone, because the rig is fixed relative to the room and the north dial must not swing it. Two further defects came out of review. The comparison is `> -GRAZE`, not `> 0`: the default mood is `day` at azimuth **exactly** 180 and the default bearing is exactly 0, so `Math.sin(Math.PI)` = 1.2246e-16 made the horizontal dot ±3e-17 against the side walls and `> 0` called one wall lit and its mirror image dark — two identical TVs on opposite walls, one casting, in the configuration every room opens in. The tie now falls toward **casting**, because a wall-rider is a box standing proud of the wall and a sun running parallel to it throws a real shadow. And **an opening is exempt**: `anchorFor('door')` is `'wall-floor'`, so `ridesWall` caught doors, windows and curtains — the places light actually crosses the wall — and removed the shadow that reads as light coming in through the doorway. That one failed in the quiet direction, a shadow silently missing rather than one wrongly present. One limitation is stated in the module rather than left to be discovered: `castShadow` is per-OBJECT, so switching it off also removes the piece from a spot lamp's shadow map at `quality: 'high'`. |
