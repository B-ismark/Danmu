...
/** RNG seeds tried per starting point. Deterministic per seed, so this is a fixed
 *  number of attempts rather than a random search. */
const SEEDS = [1, 2];

/** Where to START the piece before annealing, and this matters more than the RNG.
 *
 *  Varying only the seed is not varying the attempt: the solver's inertia term charges
 *  for movement, so every run from one origin explores the same neighbourhood. Seeding
 *  a dining chair at the centre of the largest bay, in a room whose table sits in that
 *  centre, starts it INSIDE its own anchor — and four seeds all agreed on burying it
 *  there, so the answer came back "no room" for a chair in a room with a table in it.
 *
 *  So the starts are spread across the floor the room actually has: each bay's centre,
 *  plus a point in from each side of the largest bay, all of them inside the polygon by
 *  construction because `room-bays` only returns rectangles of real floor. */
function origins(room: { footprint: Footprint }, dimMM: [number, number, number]): Array<{ x: number; z: number }> {
  const bays = roomBays(room.footprint, { max: 3 });
  if (bays.length === 0) return [centreOf(room.footprint)];
  const out = bays.map((b) => ({ x: b.cx, z: b.cz }));
  ...
  for (const side of baySides(bays[0], room.footprint)) {
    out.push({ x: side.mx + side.nx * inset, z: side.mz + side.nz * inset });
  }
  return out;
}
...
export function checkFit(
  ...
  for (const start of origins(room, candidate.dimMM)) {
    for (const seed of SEEDS) {
      withProbe[withProbe.length - 1] = { ...probe, pos: [start.x, probe.pos[1], start.z] };
      const solved = solveLayout(withProbe, room.footprint, locked, { seed });
      ...
      const seated = settleParts([...parts, posed], room.footprint, { frozen }).at(-1)!;
      ...
      const foot = footFromPart(seated.pos, seated.rot, seated.dimMM, seated.circle);
      ...
      if (overlapsSomething(foot, seated, parts)) continue;

      const settled: Placement = { x: seated.pos[0], z: seated.pos[2], yaw: seated.rot };
      const issues = analyzeRoom([...parts, seated], room).issues.filter((i) => i.partIds.includes(PROBE_ID));

      // Nothing to say about it is the best possible answer; stop looking.
      if (issues.length === 0) return { status: 'fits', placement: settled, issues, headroomMM, largestBay, outOfRange };
      if (!best || rank(issues) < rank(best.issues)) best = { placement: settled, issues };
    }
  }

  // No seed seated it inside the room at all.
  if (!best) return { status: 'no-room', issues: [], headroomMM, largestBay, outOfRange };
...
/** How bad a set of findings is, for picking between seeds. Errors dominate; ties
 *  break on count, so the arrangement that upsets the fewest rules wins. */
