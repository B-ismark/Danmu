 ...
 * **8.55-14.33 on a GitHub runner** over three runs — below the entire local range
 * rather than inside it. The third sample is the one that matters: two runs said 9.30
 * and 8.55 and looked tight enough to build on. Across those runs the yardstick spans
 * 1.06x and `referenceWorkload` spans 1.76x, so the ratio's spread is almost entirely
 * the reference term's own noise — the yardstick is steady rather than normalising.
 ...
 * bound been set from the local range — anything above 14.4, which every honest
