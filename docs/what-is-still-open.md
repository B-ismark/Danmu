...
below, recorded it as built, browser-verified, and mutation-tested. Three statements, one
file, and the two a reader meets first were the wrong ones — because a header is written by
editing the previous header, and a section is written by doing the work. **Check for the
module, not for the row:** `lib/press-selection.ts` and `lib/drag-click.ts` are both on
`main` with tests, which takes ten seconds to establish and is the whole answer.
...
- **NEW, 2026-09-05: the calibration's own self-test now fails on CI at random, and it is
  ...
  **CLOSED 2026-09-06. Neither candidate was taken, and the CI reading is why.**
  ...
  ratio=9.30`, `workload=14.39ms yardstick=1.68ms ratio=8.55`, and — on the run that
  gated the merge itself — `workload=25.27ms yardstick=1.76ms ratio=14.33`. Against
  16.6-24.3 locally that is **below the entire local range, not
  inside it** — so the ratio is not scale-free and a bound set from local numbers would
  have gone red on the very next run — and all three came in under it.

  **The third sample is why this quotes a range and not a pair, and it says something
  the ratio alone does not.** Two runs said 9.30 and 8.55 and looked tight enough to
  build on. Across the three, the yardstick spans **1.06x** while `referenceWorkload`
  spans **1.76x** — so essentially all of the ratio's spread is the reference term's own
  noise, and the yardstick is not normalising it, merely steady while the other is not.
  A second and independent reason a ratio bound fails, on top of the two workloads not
  scaling together between machines.

  And the runner's 25.27 ms is **above** this box's idle maximum of 22.58, while its
  14.39 is below the idle minimum. **The runner is not a fast machine, it is a variable
  one** — which is the honest account of the original 8.55 ms absolute failure, and it is
  not something any bound calibrated on a steady machine survives. The reason the ratio
  itself does not travel was already written in
...
12. **Room check speaks centimetres — ANSWERED and BUILT 2026-09-03: convert through
    `dimUnit`.** The user picked the middle of the three, on the stated grounds that one
    panel speaking two units is the actual defect. `formatLength` (`lib/units.ts`) is the
    one formatter every finding goes through, and `analyzeRoom` gains a `dimUnit` option.

    **It defaults to `'cm'` rather than to the store's `'m'`, and that is not a
    half-measure.** `lib/clearance.ts` is pure and must not import a store; the default
    keeps the two callers that never show a finding to anyone — `lib/fit-check.ts` and
    `lib/layout-shuffle.ts`, which read `rule` and the issue key — on the sentences they
    have always produced, so a solver comparison cannot start depending on a Settings
    value. `RoomTools` is the one caller that renders `detail`, and it passes the real
    unit.

    **Three things a straight `fromMM` would have got wrong, each of which is the same
    scar one layer along from where `boundsToUnit` already carries it:**

    · **A coarse unit rounds a real measurement to zero.** A 4 mm gap at
      `precisionFor('m')` is `0.00`, and *"Only 0.00 m between the sofa and the table"* is
      a false statement about a real clash — read as the app being broken rather than the
      number being rounded. The decimals grow until the number is true, capped at one
      millimetre of resolution, DERIVED from the unit rather than typed. For `mm` the cap
      is zero extra places and the loop never runs.
    · **Trailing zeros are trimmed**, which is the one place this parts company with
      `formatDim`. A field is a column and wants fixed width; `RoomDimsEditor` fills its
      inputs with `String(fromMM(...))`, so the field shows `198` and a finding saying
      `198.0 cm` beside it would have been a NEW disagreement introduced by the fix for
      the old one.
    · **The mounted-clash band still rounds OUTWARD**, low floored and high ceiled, which
      is what stops a 7 mm band collapsing to one number now that metres are reachable.
      Asserted across all five units in `tests/mounted-clash.test.ts`; metres renders it
      1.05 .. 1.07.

    `cachedReport` in `RoomTools` keys the whole report on its inputs, so `dimUnit` joined
    that key — an input that changes the value without changing the key is a panel that
    goes on saying the last unit until something else happens to move.

    **Two existing tests asserted hand-typed centimetre strings and both were slightly
    untrue.** `zone-findings` asserted `has 4 cm in front` over a comment claiming
    "40 mm, derived": 4 was typed, and it is `Math.round` over a gap of 39.x mm. It asserts
    now that every unit describes ONE length, with the tolerance derived from
    `precisionFor`. `mounted-clash` captured `(\d+) cm`, which against "105.6 cm" matched
    the "6" — a mis-parse that reported as the band being inverted, i.e. as the very defect
    the test exists to raise.

    **Mutation-verified 16/16**, after a first pass left the decimal-growth loop entirely
    unguarded: deleting it kept every suite green.

    **Committed:** on `fix/rider-height-and-report-units`. The original framing follows.
...
16. **A rider floats after a reload — ANSWERED: derive at read time, write nothing. BUILT
    2026-09-03, on the THIRD attempt.** The user picked the second option.
    `lib/rider-height.ts`'s `resolveScene` — which is `resolveParts` plus the
    correction, and is deliberately NOT `resolveParts` itself — applies
    `deriveRiderYs`; nothing persists and a re-detect stays clean. (This line read
    "`resolveParts` applies `deriveRiderYs`" for two attempts, which contradicts
    `lib/transforms.ts`'s own comment: a reader greps `resolveParts` for the
    correction, finds none, and concludes the fix was reverted.) See § H.12 for what it
    cost, and for the one thing this paragraph's wording turned out to settle: **"derived
    from its support's *current dims*" is narrower than "settle whatever floats"**, and
    the first attempt read it as the wider thing and deleted § 37's *Floating* report. The
    original framing follows, because the two rejected options are the useful part of the
    record.
...
17. **Does the placement row earn its place? — RESOLVED 2026-09-03, and the answer's own
    premise had expired.** The recorded answer was *"dragging would work"* — keep the
    operations, drop the row — and it rested on this section's claim that **neither
    Floor-off-a-table nor Surface-back-onto-it is reachable by dragging**.

    That stopped being true when the drag pipeline moved into `lib/drag-resolve.ts`, and
    nobody re-derived it. Measured against `resolvePlacement`, which is the one resolve
    both tabs run: a lamp dragged clear of a desk lands at **y = 0** with no support, and
    dragged back over it lands at **0.75** with `supportId` naming the desk. Surface is a
    drag, exactly.

    Put back to the user with that measurement rather than executed on the stale one.
    Their answer: **drop Surface only.** The other two are the two a drag cannot express,
    which is why the row survived rather than being deleted as the original answer's
    letter would have had it:
    ...
    · **Wall** moves the piece to the nearest wall AND turns it to face the room.
      ...
      curtain family — so a sofa is never slid onto plaster and never rotated. Measured: a
      sofa dragged to z = 1.4 with snap on keeps its 0.9 rad yaw and stops 0.84 m short.
    · **Floor** puts it on the floor WITHOUT moving it in x/z. Dragging it clear also
      drops it, but somewhere else, and "on the floor, under the desk" is only this.

    `supportBelow`, `snapToSurface` and the `snap-surface` glyph went in the same commit,
    because a helper or an icon kept for a control that no longer exists is the
    dead-plumbing shape `CLAUDE.md` rule 1 describes. The row is two buttons at 50%
    always, rather than two or three.

    **This is the fourth time in this document a hand-off claim has been acted on and
    found stale**, and the first where the stale claim was inside an answer the *user* had
    given: an answer is only as good as the premise it was asked on, so re-deriving the
    premise is part of executing the answer rather than second-guessing it.

    `tests/where-it-sits.test.tsx` keeps `placementButtons()` scanning for all three words,
    so a re-added Surface fails rather than passing unnoticed, and the drag-reachability
    measurement is an assertion in that file now — the button cannot be removed on a stale
    premise twice. Its deleted Surface test was the negative control for Floor, so Wall
    took that job. **Mutation-verified 16/16**, after a first pass found `groundToFloor`'s
    `clearParent` unguarded: the fixture never gave the lamp a parent, so removing the call
    changed nothing.
...
**Nothing is left of § E.** This sentence said the § H.8 drill-in repro was, and that was already false when it was written: the drag half was measured in `lib/` and built (#111, #113) with no shim — the second time this file filed something as DOM-only that was arithmetic — and the drill-in half was built as `lib/press-selection.ts`, in a browser, with the diagnosis recorded in § H.8 below. **A closed item hiding inside an open one is how this file rots**, and it rots in the direction that costs the most: the next reader builds what already exists. Derive it by looking for the module, not by reading the row.
...
than a case for a hull. ~~`desk-l` draws 2.86 m where every consumer reads 1.60 m~~ **FIXED 2026-09-06**. The larger
...
**2026-09-06 — the "32 of 46" number is now GATED, and two of the three worst are closed.**
That count had printed on every green run since the table existed and failed nothing, which
is this repo's recurring shape: not a broken check, an unread one. `footprint-fidelity` now
carries a per-shape overhang budget — `DEFAULT_OVERHANG_MM = 60` with a named exception
table, asserted for **every** shape rather than for a threshold with a list excused from it,
and the exception table's own key set is pinned so adding a row cannot be the cheap way to
green a red.

Measured at the `lib` row only, and that is a real distinction: for a shape outside
`PARAMETRIC_SHAPES` the renderer is authored once and a resize arrives as a uniform group
scale from `Draggable`, so `min` and `max` are not configurations the app ever draws.
Asserting over all three would pin two meaningless rows per shape and then have to be
widened for them.

- **`desk-l` is fixed.** The return arm was authored beyond the box's right edge while the
  main tabletop already filled the whole `w × d`, so there was never room for an L inside
  the declared footprint. `overX` 1260 mm → **0** at all three sizes; `desk-standard` is
  untouched by construction (`armD === d` when `lShape` is false). Mutation: pushing the
  arm back out is caught with `desk-l 672.0 mm > 60`.
- **`laptop` is budgeted at 70 mm, not fixed, and it is the honest kind.** The lid hinges at
  the back edge and tilts −0.34 rad, so an open screen leans behind the base — which is what
...
exactly, `chair-dining` 0.71, `chair-office` 0.74, and `lamp-floor` / `lamp-table` escape by
30 mm and 15 mm. Only `plant` is materially wrong. The "six renderers that never read
`dimMM`" framing counts renderers; what matters is millimetres at the authored size, and
those two lists are not the same list.
...
### 12. A rider stays at the size the room was BUILT at — BUILT on the THIRD attempt, PR #100.

**Where it rides.** `main`, `04ce6e0` (PR #100 merged 2026-09-03): `lib/rider-height.ts`,
hooks in `lib/room-scene.ts`, `Draggable`, the scene-file export, and
`coversEnoughToSupport` extracted into `lib/physics.ts`.

**The core case has now been measured in a browser**, which is the one thing no test here
can do. Headless Chromium against a production build, U-Shape preset, nightstand
`0.55 → 0.75 m`: the bedside lamp on it rose **+0.200 m in the drawn scene graph**, the
lamp on the *other* nightstand did not move, and nothing else in the room did either;
typing the height back put it back. **The same probe against `2f3d0dd` — `main` one commit
earlier — leaves the lamp at 0.55, 200 mm inside the nightstand**, so the probe was watched
failing rather than merely passing. `docs/visual-check.md` carries that table, the eight
things still wanting a person, and the scene-graph route (`__THREE_DEVTOOLS__` plus
`userData.danmuPartId` — **not** `canvas.__r3f`, which does not exist).

**What the third attempt cost, so the number is on the record.** Five lenses again, on the
branch that was supposed to be right: **twenty-five more findings**, fifteen of them code.
Three lenses independently found the same top one — `SceneFile.tsx` was passing four of the
five transform slices, so the export half of the fix was inert and my own gate for it was
green because its fixture was a *seeded* rider, the one case that survives the omission.
Two mutation rounds: 23/23 then 21/24, with the three survivors closed and one of them
proven an equivalent mutant rather than a gap.

**The three that changed the design, beyond the nine below.**

1. **The gate asked a proxy question and was wrong in both directions.** *Has this
   support's top moved since it was authored* is too weak for an edge a drag RECORDED —
   every consumer that moves a piece in x/z copies `pos[1]` out of the resolved scene, so
   setting the support's height back to exactly its authored value stranded the rider at
   the baked Y, 450 mm in the air and persisted. And too strong for an edge merely
   INFERRED: `clearParent` deletes a key and cannot record *rides nothing*, so pressing
   **Floor** on a lamp whose nightstand you had already resized put it on the floor and the
   pass put it straight back. Two clicks, and no lens found that one — it turned up while
   refuting a simpler fix. It is two rules now: `pos[1] <= 0` rides nothing, and a recorded
   edge is unconditional while an inferred one stays gated.
2. **It cost `2N + 8` whole-room derivations per store write**, because `Room.tsx` mounts a
   `Draggable` AND a `Dressing` per part and both read `useSettledY`. Measured 14.3 ms per
   drag frame at 60 parts against a 16.7 ms budget. `riderYs` caches on reference identity;
   `useSettledY`'s selector returns a NUMBER so a part re-renders only when its own Y
   changes. The docblock claiming an early return made it free was measured and **false** —
   94% of the cost is spent before that return is reached.
3. **A clamped rider loses its relation across a save.** The ceiling clamp deliberately
   leaves a rider off its support's top, which is the one thing `ridingParents` needs to
   re-infer an edge. `buildSceneFile` writes the relation from `riderRelation` rather than
   `parentIds`, so a seeded or clamped rider is not orphaned.

**The nine from the second attempt are kept below**, because the design still answers them
and because the two retracted attempts are the useful part of the record.
...
gravity pass. It fixed § 12 and it also **deleted a feature that had shipped three commits
earlier**: § 37's placement banner reports a piece resting on nothing as *Floating*, and
...
**Not verified:** nobody has resized one of these on screen — still literally true, and left
standing for that reason. What a probe DID settle is narrower: seeded at two declared sizes each,
six shapes drew their declared size **14 times out of 14 at ratio 1.00** off the three.js graph
(`4cef13a`). That is the geometry answering, not the resize CONTROL, which nobody has touched.
See `docs/visual-check.md`.
...
of the original complaint — three beds at `pos [0,0,0] rot 0`, stacked and all facing the
same way — had one cause: with no aim `placeNewPart` reads `ax = az = 0`, so the piece went
to the middle AND asked which wall was nearest the middle, which is the same wall every
time. Moving the aim fixed the heading for free.

The review of that fix found four defects in it, all now closed and all listed in the
commit: the ring was centred on the **world origin** rather than the room (four chairs on
one point in a room whose walls had been dragged — the same defect inside the function
written to close it); the probe **dropped roundness**, reopening § 32 one function over;
repeated clicks built a **tower** (four floor lamps at y = 0.00, 1.50, 3.00 and 4.50 in a
...
wrong for a plant on a plant. Only the physically impossible half is gated (§ 31: nothing
may be based where its top passes the ceiling). Whether an UNAIMED click should stack at all
...
### § 42.1 · A floor piece was decoded at its near face — **FIXED 2026-09-09**

`placeFloorObject` backprojected the bbox's bottom edge onto the floor plane. For a
fronto-parallel card that edge is the object's centre line; for a real box it is the corner
**nearest the camera**. So every floor-standing piece was measured about half its own depth
too close to the lens, and a piece with a square footprint additionally read far too wide,
because its silhouette is its diagonal.

Why nobody saw it: `bboxOfFloorObject` built its corners by offsetting along the
wall-parallel axis **only**. A 600 mm-deep wardrobe was a flat card, so the placer's answer
was exactly right about the thing it was given, and the zero-error baseline in
`detect-pipeline.test.ts` never described furniture with depth.

**The decision that unblocked it, taken by the user 2026-09-09:** fix the whole silhouette
rather than only the position, and take the depth from the **catalogue** —
`defaultDepthFor(category, shape)` — never from the detector, because `depthM` moves a
POSITION and a depth the AI guessed would be an AI-decided placement. `geoRefine` writes that
same number into `dimMM[1]`, so a floor piece is drawn with the depth it was placed by; a hint
kept for the render beside a default used for the maths would have left the two disagreeing by
half their difference on the one axis the photo did measure.

**What it rests on.** A floor point's image row is a function of its forward distance ALONE —
the lens rotates about its own right axis, so `tanX` is untouched by tilt and two floor points
at the same distance share a row to twelve digits. So the lowest row in a silhouette is the
near face, exactly, at any tilt. Everything else follows in closed form: forward is
`near + depth/2`; the height reads at the near or FAR top edge depending on whether the
piece's top is above the lens; each silhouette edge is the box corner that `forwardAtHeight`
makes nearest or furthest. A round footprint takes the tangent form instead and recovers its
own diameter.

**Two things fell out that nobody had measured**, and both are the same root cause:

- A piece whose top is **below** the lens images its FAR top edge, so reading the height at
  the near face made both nightstands ~130 mm too tall and the sofa ~150 mm. Invisible
  because the printed baseline table had no height column. It has one now.
- **Tilt roughly doubles the width inflation.** A floor lamp read +81% too wide at a level
  lens and +132% at 5°. That matters more than it used to: Phase 1 restored the sensor grant,
  so `tiltRad` is genuinely populated on live captures now.
...
**The one approximate term, stated rather than smoothed.** A vertical tangent line's image
column varies with the row, and the row at which tangency actually falls is not the bbox's own
top row, so under tilt the round branch reads its azimuths a little off. A fixed-point
refinement was tried and does not converge for a tall thin cylinder, so it is not shipped on
the strength of a guess. Bounded on BOTH sides in
`tests/photo-geometry.test.ts` — a floor as well as a ceiling — because a bound that only caps
an error cannot notice it being fixed, and a stale "approximate" note is how a solved problem
stays open.

**Two clamps, and the mistake in between.** The near face is measured, so it is bounded by the
plaster; the centre is measurement plus assumption, so it gets its own bound — the piece's back
may reach the wall and no further. Folding the depth into the first clamp instead is the obvious
one line and it shrank a MEASURED 2.0 m sofa to 1.925 m: an assumption corrupting an
observation, traded for an exact position. Found by measuring, not by reasoning about it.

**Does it exist in a commit?** Yes, with the fixture that can express it. Ten mechanism
mutants were run and all ten fail the suite: the `depth/2` push negated and removed, the
corner extremes swapped, the top-row face choice dropped, the round branch routed as a box,
the catalogue depth swapped for the AI hint in both places, roundness never routed, the two
clamps folded into one, and the round branch's tilt-aware edge row flattened. Four guards
...
| fixture (6 × 4 room, the one the tests use) | truth | decoded before |
|---|---|---|
| 700 × 500 print on the N wall, centre 800 mm from the NE corner, in the E photo | on another wall | **764 mm past a 2.0 m wall end, 893 × 803 — +28% wide, +61% tall** |
| 1400 × 500 curtain, far edge 250 mm from the same corner | on another wall | **860 mm past, 1815 × 942 — +30% and +88%** |
| 300 mm vent high on the N wall, read as ceiling in the E photo | not on the ceiling | **passes the old 3.0 m gate at 2.56 m, 571 mm outside the room, 386 mm wide** |

**Every one of those numbers replaces a published figure that was wrong**, and the way it
was wrong is now a `docs/traps.md` entry: the originals (960 × 711 at 769 mm; the ceiling
case at 3.30 m / 643 mm / 401 mm) came from a scratch script that re-implemented the
placer's arithmetic in node rather than calling it — a careful measurement of the wrong
subject — and they described a 7 × 6 room while the shipped fixtures use 6 × 4. One
sentence in the commit body welded a 7 × 6 displacement to the 6 × 4 room's bound. The
height error, +61%, is the larger of the two and no version of the note mentioned it.
`tests/photo-geometry.test.ts` prints the table on every green run now, which is the only
thing that stops this recurring.
...
still reaches the scene, and `label-repair` reads that same identity as "unmeasurable".

**Two things that sentence used to add, both false, both corrected after review.** *"At its
catalogue size"* — only on the on-device path. `buildSceneFromRoom` prefers the detector's
own `dimMM` through `clampDims` and reaches `cfg.dim` only when there is no hint at all, and
the cloud prompt asks for `dimMM` — so a refused cloud detection is drawn at the AI's clamped
guess. And *"`judgeLabel` does not accuse it"* — for the print there was no accusation to
prevent: `painting`'s band is 150–2400 × 150–1800, so a fabricated 893 × 803 was judged
**`ok`**, a false clean bill, and the refusal WITHDRAWS it (`ok → unmeasured`). The row that
really was accused is the vent, 386 mm against `fan`'s floor.

**Still open, and it is the trust-boundary half:** should a refused wall or ceiling detection
also DISCARD its AI `dimMM`, so the fallback really is the catalogue? It would make the
refusal mean what its own prose claimed, but it throws away a hint the app trusts everywhere
else and that `clampDims` already bounds. Not taken; wants a decision.
...
- **"The gate removes a duplicate row."** It does not. The print is in the north photo
  ...
  to move, and `tests/detect-refine.test.ts` now pins that. **The mechanism half is
  fixture-dependent, which the first write-up did not say:** the merge skips a row whose
  position is *missing*, and the cloud prompt asks for `position`, so a refused cloud row
...
calibration — and BOTH directions are measured now, where the first write-up named only the
harmless one.** A wall piece's decoded offset is *exactly* proportional to `cal.k`, since
the distance is pinned to the wall and `lateralSpan`'s multipliers are room-derived, so the
refusal threshold moves as `1/r` for `r = k_believed / k_true`. Printed by
`tests/photo-geometry.test.ts` on every green run — the largest fraction of the half-span
still accepted:

| the shot was | read as | r | last accepted | size already wrong by |
|---|---|---|---|---|
...
So an **under-read** lens never refuses a real piece: it pulls every lateral inward, which
hides a fabrication where nothing can see it — the same print read at 66° instead of 106°
lands 2.27 m out, comfortably inside a 3.0 m half-span — and touches nothing legitimate.
An **over-read** one refuses further in the wider the error: 66° mistaken for 106° costs
the outer HALF of every wall. **The last column is what makes that defensible**, and it is
an argument nobody could make before measuring: width comes off the same tangents at the
same pinned distance, so an `r` that moves the threshold has already inflated the size by
the same factor — the piece refused at 0.45 of the wall would have come back 104% too wide.
The over-read case therefore discards a measurement that was already worthless, which is
not a bound overruling a measurement. So this closes the case where the lens is known, and
its failures where the lens is wrong are asymmetric by construction.
...
**The mutation battery after the review, 30 mutants over the code and the fixtures, and
the three things it found are worth more than the count.** (i) *One of the plan's own
mutations was invalid.* It called for the sweep's coverage literals to be **loosened** to
`> 0` and said that must fail — but loosening an assertion can never fail a green run, so
it "survived" by construction. The real checks are the literal itself (`toBe(60)` →
`toBe(59)` fails) and three mutations of the SUBJECT that lose rows — a slot, a tilt, the
±0.99 ends — each of which the literals catch and floors would not. *A mutation that can
only pass is the same defect as an assertion that cannot fail, one level up.* (ii) The
asymmetric bound was **unpinned**: `Math.abs(right) <= frame.right` agrees with the real
bound on every centred room and on both off-centre rows, so it survived — the whole point
of reading a polygon rested on fixtures that could not tell the two apart. A fabrication
at x = −3.4 past that wall's short end kills it, and the one-sided variants with it.
(iii) `if (!frame) return false` survived too, because every other fixture hands over a
real rectangle; a two-point footprint now pins that **no answer must not become a
refusal**. Two survivors are left and both are recorded rather than papered over: adding
the gate to `placeFloorObject` (the documented one — the exemption rests on the measured
lens-invariance above, not on an assertion), and flipping the bound's two comparisons to
strict, which nothing can observe because no fixture puts a centre exactly on a bound.
...
### 44. The placers measured from ±width/2 — FIXED 2026-09-10

**FIXED.** `wallDistance` is **deleted**. All five sites that measured from the ±half pair
— `calibrateFromFloorLine`, `heightFromFloorLine`, `placeFloorObject`, `placeWallObject`,
`placeCeilingObject` — read `wallFrame(slot, footprint)`, and their signatures take a
`Footprint` rather than a room, so a bounding box is **unpassable** rather than merely
unread. `wallSpan` survives, read only by the capture screen's label. The gate's inertness
rule is deleted with the input that justified it.

**The inventory was one site short of what this item claimed.** It named four; there were
five. `heightFromFloorLine` also read the pair and is on the live path
(`app/onboarding/detect/page.tsx`).

**It was reachable, which this item under-stated as "only after a wall drag" without
tracing whether a dragged room can be re-scanned. It can.** `offsetWall` translates two
vertices and copies the rest (*"the opposite wall does not move, so the room becomes
off-centre — that's intentional"*), `moveWall` stores that polygon raw and re-derives
width/depth from its new bbox, `RoomSync` persists it, and nothing anywhere recentres a
footprint — the only three footprint writers are `footprintForLayout` (centred),
`offsetWall` (not) and `readFootprint` (arbitrary). Then `PartTree` offers **Re-scan** as
the Room section's header action, unconditionally, and `RoomSync` deliberately declines to
pin the scene of a reshaped room that still has photos *so that re-scan keeps working*.
Drag a wall, press Re-scan, and every one of the five gets the off-centre polygon.

**What it cost, measured by calling the functions** — a 700 × 500 print on a 6 × 6 room's
north wall dragged out to 4.0 m, bounding box 3.5. Printed by
`tests/photo-geometry.test.ts` on every green run, per the trap this thread filed about
numbers taken from re-implementations:
...
| EXIF + § 44 | 700 | 500 | 0.800 | 3.985 |

**The two findings worth more than the fix.** First: **the error CANCELLED on the
floor-line path.** `calibrateFromFloorLine` solves `k ∝ 1/d` from the same assumed distance
the placers then multiply back by, so a dragged room's wall SIZES were already right (699 ×
499) while the distance was 500 mm out. That is why nothing caught this — and a cancellation
is a trap rather than a reprieve, because **migrating either half alone breaks it**, +14%
one way and −13% the other. It is the concrete reason "one mechanism per commit" is about
the mechanism and not the file count. Second: on the EXIF or vanishing-point path there was
never a cancellation to lose, `k` being known independently, so **every wall piece was
simply 13% wrong** — and `CLAUDE.md` records that not one of four real Pixel photos carried
a focal length, so which of those two paths a user is on depends on whether the floor line
solves. `heightFromFloorLine` had no cancellation either and solved a 1.5 m camera as
**1.3125 m**, 187 mm low, which then feeds every placement.

**"What does a site do when the polygon cannot answer" was the question this item was
parked on, and it is three questions.** `wallFrame` refuses a polygon under three points,
one with a non-finite vertex, or one the lens does not stand inside — reachable through the
share format, since `readFootprint` accepts any 3–256-point polygon with no origin check:

| role | sites | no frame → |
|---|---|---|
| **premise** — the plane or distance solved from | both solvers, `placeWallObject` | `null`. No honest substitute exists for a plane; the bounding box *was* the substitute |
| **gate** — a bound on an assumption located elsewhere | `placeCeilingObject`'s forward gate, `onFramedSurface` | inert. `room.height` locates the slab and no footprint can move it |
| **clamp** — a bound on a measurement | `placeFloorObject`'s two | inert, while the 0.3 m arithmetic guard stays |

The premise row is not a regression from § 42.3's *"no answer must not become a refusal"* —
that sentence was about a GATE. For a plane, refusing still costs the measurement and not
the piece: `geoRefine` hands the detection back and `label-repair` reads that identity as
unmeasurable.

**The gate's inertness rule is gone, and its deletion is asserted as the payoff rather than
as a removal.** The return-wall fabrication is now REFUSED in a framed-wall-dragged room,
where it was accepted for as long as two conventions disagreed. A second finding fell out
of the same swap: `placeCeilingObject`'s forward gate was reading the bounding box, so in a
room whose north wall was dragged OUT, every legitimate ceiling piece between 3.5 and 4.0 m
was **refused** — a gate discarding correct measurements, which is the defect § 42.3 was
itself corrected for, one axis over. Mutation found it; nothing in the suite could see it
while every fixture was centred.

**Verification.** `detect-pipeline` and `off-square-cost` baselines **byte-identical** (the
no-op proof on centred rooms). `wallDistance` deleted and the suite compiles (the
completeness proof — nothing was left on the old convention). **13 mechanism mutants, all
13 caught**, plus two FIXTURE mutants (the dragged room reverted to centred; the drag
direction flipped) at 8 failures each.

**Two of my own assertions were defective and were fixed by mutation, not by review.**
`expect(distance).toBeGreaterThan(0.3)` was appended to a test whose fixture decodes at
1.4 m — true of any implementation at all, so moving the 0.3 m guard inside the frame check
survived a full green run. It is its own test now, on an input where the guard actually
bites (a 66° lens, 0.8 m camera, 0.9 rad tilt, a box at the frame's bottom). And a floor
fixture passed `hM`/`depthM` transposed, so it recovered the right distance *because the
clamp fired* rather than because the measurement was right. Both are the same shape as the
...
So **three walls of sixteen had the wrong DISTANCE and five more had the wrong ENDS.** The
ends are the surface gate's input, so those five handed § 42.3's gate 2–3 m of return wall
and told it that was the framed one — the fabrication that gate exists to catch, in the
rooms it could not see. The `l` is the sharp case: its distances are right, so nothing
about those photographs looks wrong from any other angle.
...
every placement with it. Filed, not smoothed — and note that the `u`'s east and west views
are taken 1.32 m from the notch's side walls, close enough that the wall-floor junction
sits at `v ≈ 1.07`, off the bottom of the frame, so the floor-line rung cannot fire there
either.
...
**Mutation: 16 mechanism mutants, 16 caught — but only after two of them earned their
keep.** One SURVIVED (*the farthest facing candidate instead of the nearest*), because
every ray out of a preset leaves the room exactly once, so the two rules agree on every
shipped shape; the fixture that separates them is a footprint whose arms wrap around the
view axis, where the axis exits at 1 m, crosses outdoors and exits again at 7 m. Two more
were **provably equivalent**, and chasing them changed the CODE rather than the tests: a
flat-wall special case whose removal changed nothing (`x + (0·y)/z` is `x`), deleted — and
with it a comment claiming that branch was what made the no-op proof exact, which was
false; the bit-identity comes from reading the wall's own vertices at all. The edge-on skip
is the other, and it stays with its equivalence written down, because the behaviour would
...
100% visible for all five presets so nothing ships in that state; and an OBLIQUE wall has
