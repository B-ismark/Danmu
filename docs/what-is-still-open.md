...
one obstacle at 4096 positions a shape, the drawn geometry refuses 1100 positions the box
allows and the box refuses 293 the geometry allows — and **every one of the 1100 traces to a
renderer drawing outside `dimMM`**, which is a literal in the wrong place rather than a case
for a hull. `desk-l` draws 2.86 m where every consumer reads 1.60 m. See § 4.6 of
`docs/research/suggest-and-collision.md` for the tables; the instrument is
